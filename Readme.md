...
>Note: The resulting document is exported by rasterizing each page to an image / slide (no native PowerPoint inner elements). This example uses the `Microsoft.Office.Interop.PowerPoint` library, that requires a local installation of PowerPoint. You can also use the similar solution with any other library that supports generating Power Point documents.
